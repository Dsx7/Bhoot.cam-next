...
export default function Explore() {
  ...
  const [loadingMore, setLoadingMore] = useState(false); // Button Load
  const [hasMore, setHasMore] = useState(true); // Check if more exist

  // 1. Reset & Fetch when Category changes
  useEffect(() => {
    setEpisodes([]);
    setPage(1);
    setHasMore(true);
    ...
    <div className="min-h-screen bg-[#050505] text-white">
      <div className="container mx-auto px-4 py-12">
        ...
        {loading ? (
           ...
           <>
             {/* Note: Grid matches "Single on Mobile" requirement */}
